...
static  I32  __MR_ExtendFieldsToMultiVaraiteTS(FIELD_ITEM *flist,I32 N,I32 q) {
	...
	for (int i=0; i < N; i++) {
		...
		for (int j=0; j < q; j++) {
			snprintf(qList[j].name,63,"Y%d",j+1); 	
			qList[j].extra=0;
...
void* BEAST2_Output_AllocMEM(A(OPTIONS_PTR)  opt)  {	
	...
	I32    nfields=sizeof(fieldList)/sizeof(FIELD_ITEM);
	int sig2_index=6 - 1;
	__AddSpatialDimension(io->dims[io->rowdim - 1],io->dims[io->coldim - 1],io->out.whichDimIsTime,&fieldList[sig2_index],1L); 
	...
	*result=(BEAST2_RESULT){0,}; 
	...
	if (hasSeasonCmpnt) {
		...
		if (opt->extra.computeSeasonAmp) {
			nodes[nid++]=(MemNode){ &result->samp,sizeof(F32) * Nq,.align=64 };  
			nodes[nid++]=(MemNode){ &result->sampSD,sizeof(F32) * Nq,.align=64 };  
		}
	}
	if (hasTrendCmpnt) {
		...
		if (opt->extra.computeTrendSlope) {
			nodes[nid++]=(MemNode){ &result->tslp,sizeof(F32) * Nq,.align=64 };  
			nodes[nid++]=(MemNode){ &result->tslpSD,sizeof(F32) * Nq,.align=64 };  
			nodes[nid++]=(MemNode){ &result->tslpSgnPosPr,sizeof(F32) * Nq,.align=64 };  
			nodes[nid++]=(MemNode){ &result->tslpSgnZeroPr,sizeof(F32) * Nq,.align=64 };  
		} 
	}
	if (hasOutlierCmpnt) {
